	         	...
	         	<section id="content">
	            	<ul>
			            <c:if test="${listcheck != 'empty' }">
			               	<c:forEach items="${list3}" var="list3">
				               	<li>
				                  	<div class="goodsThumb">
				                     	<img src="${list3.gdsThumbImg}">
				                  	</div>
				                  	<div class="goodsName">
				                     	<a href="/shop/view?n=${list3.gdsNum}">${list3.gdsName}</a>
				                  	</div>
				                  	<c:if test="${list3.gdsRate == '0' }">
                                 <div class="goodsPrice">
                                    상품가격: <fmt:formatNumber pattern="###,###,###" value="${list3.gdsPrice}"/> 원
                                 </div>
                                 <div>
                                 &nbsp;
                              </div>
                              ...
                              <c:if test="${list3.gdsRate != '0' }">
                                 <div class="salePrice">
                                    할인가격: <fmt:formatNumber pattern="###,###,###" value="${(list3.gdsPrice*(100-list3.gdsRate))/100}"/> 원
                                     <span class="background">(<fmt:formatNumber value="${list3.gdsRate}" />%)할인</span>
                                 </div>
                              </c:if>
	               	...
	               	<ul class="pageMaker">
		                ...
		                <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="num">
		                   	<li class="pageMaker_btn ${pageMaker.cri.pageNum == num ? 'active':''}">
		                      	<a id="atag" href="/shop/list3?pageNum=${num }">${num}</a>
		                   	</li>   
		                </c:forEach>
